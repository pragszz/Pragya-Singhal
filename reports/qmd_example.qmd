...
![Horse populations for all provinces in Canada from 1906 - 1972.16.](../results/horse_pops_plot.png){#fig-horse-pop width=95%}
...
Additionally, note that in Table \@ref(tbl:horses_sd) we consider the sample standard deviation of the number of horses during the same time span as Figure @fig-horse-pop).

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-horse-pops-plot-largest-sd width = 70%}

In Figure @fig-horse-pops-plot-largest-sd we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
